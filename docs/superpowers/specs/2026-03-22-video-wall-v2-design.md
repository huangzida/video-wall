...
- 单选：点击激活并置顶。
- 多选：Shift 点击追加/取消；Shift+拖拽进入框选多选。
- 锚点区间选：点击激活窗口后自动更新锚点。

与现有框选建窗语义兼容规则：
- 空白区域拖拽默认行为保持不变：创建窗口。
- 仅在按住 Shift 的情况下，空白区域拖拽才进入“框选多选”模式。
- 若后续需要显式模式切换（Create/Select），作为可选配置项，不改变默认行为。

锚点区间选定义：
- 锚点为最近一次“主选中窗口”（非追加选择的最后焦点）。
- 单选或首次多选时，Shift+点击执行追加/取消。
- 已存在多选且锚点存在时，Shift+点击执行区间选。
- 区间选中范围按“空间顺序”计算，默认排序为先 y 后 x。
- 可选排序策略：createOrder、zIndexOrder。
- 示例：锚点为窗口 A，Shift 点击窗口 D，则在当前排序序列中选中 A 到 D 的闭区间窗口集合。
